...
class RentalViewSet(ModelViewSet):
    ...
    queryset = (
        Rental.objects.select_related("landlord")
    ...
    def get_queryset(self):
        ...
        if search_params.is_valid():
            ...
            if data.get("max_distance_to_campus"):
                queryset = queryset.filter(distance_to_campus__lte=data["max_distance_to_campus"])

            # Availability filter
            if data.get("available_from"):
                queryset = queryset.filter(available_from__lte=data["available_from"])
    ...
    @extend_schema(
        ...
        parameters=[
            ...
            OpenApiParameter(
                "parking_available", OpenApiTypes.BOOL, description="Parking available"
            ),
        ],
        tags=["Rentals"],
    )
    ...
    def toggle_favorite(self, request, pk=None):
        ...
        favorite, created = RentalFavorite.objects.get_or_create(user=user, rental=rental)

        if not created:
            favorite.delete()
            return Response({"message": _("Rental removed from favorites"), "is_favorited": False})
        else:
            return Response(
                {"message": _("Rental added to favorites"), "is_favorited": True},
    ...
    def favorites(self, request):
        ...
        favorites = RentalFavorite.objects.filter(user=request.user).select_related(
            "rental__landlord"
        )
    ...
    def my_properties(self, request):
        ...
        rentals = Rental.objects.filter(landlord=request.user).prefetch_related("images", "reviews")
...
class RentalImageViewSet(ModelViewSet):
    ...
    def set_primary(self, request, pk=None):
        ...
        return Response({"message": _("Image set as primary successfully")})
...
class RentalInquiryViewSet(ModelViewSet):
    ...
    def reply(self, request, pk=None):
        ...
        serializer = RentalInquiryReplySerializer(inquiry, data=request.data, partial=True)
...
def featured_rentals(request):
    ...
    rentals = (
        Rental.objects.filter(is_featured=True, status="available")
...
def recent_rentals(request):
    ...
    rentals = (
        Rental.objects.filter(status="available")
        .select_related("landlord")
...
def rental_statistics(request):
    ...
    property_types = (
        Rental.objects.values("property_type").annotate(count=Count("id")).order_by("-count")
    )

    stats["property_type_stats"] = {item["property_type"]: item["count"] for item in property_types}
    ...
    for i in range(12):
        ...
        monthly_stats.append(
            {"month": month_start.strftime("%Y-%m"), "rentals_created": monthly_count}
        )
...
class AdminRentalViewSet(ModelViewSet):
    ...
    def update_status(self, request, pk=None):
        ...
        return Response({"message": _("Status updated successfully"), "status": rental.status})
