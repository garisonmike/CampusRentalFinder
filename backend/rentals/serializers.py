...
class RentalListSerializer(serializers.ModelSerializer):
    ...
    def get_primary_image(self, obj):
        ...
        if primary_image:
            return RentalImageSerializer(primary_image, context=self.context).data
        # Fallback to first image if no primary image set
        first_image = obj.images.first()
        if first_image:
            return RentalImageSerializer(first_image, context=self.context).data
        return None
    ...
    def get_is_favorited(self, obj):
        ...
        if request and request.user.is_authenticated:
            return RentalFavorite.objects.filter(user=request.user, rental=obj).exists()
        return False
...
class RentalCreateSerializer(serializers.ModelSerializer):
    ...
    def validate_available_from(self, value):
        ...
        if value < date.today():
            raise serializers.ValidationError(_("Available date cannot be in the past."))
        return value

    def validate_lease_duration_max(self, value):
        """Validate maximum lease duration."""
        if value and value < 1:
            raise serializers.ValidationError(_("Maximum lease duration must be at least 1 month."))
        return value
...
class RentalInquirySerializer(serializers.ModelSerializer):
    ...
    def validate_preferred_move_date(self, value):
        ...
        if value and value < date.today():
            raise serializers.ValidationError(_("Preferred move-in date cannot be in the past."))
        return value
...
class RentalInquiryReplySerializer(serializers.ModelSerializer):
    ...
    def validate_landlord_reply(self, value):
        ...
        if not value.strip():
            raise serializers.ValidationError(_("Reply message cannot be empty."))
        return value
...
class RentalSearchSerializer(serializers.Serializer):
    ...
    query = serializers.CharField(
        required=False, help_text=_("Search query for title, description, or address")
    )

    city = serializers.CharField(required=False, help_text=_("Filter by city"))

    state = serializers.CharField(required=False, help_text=_("Filter by state"))

    property_type = serializers.ChoiceField(
        choices=Rental.PROPERTY_TYPES, required=False, help_text=_("Filter by property type")
    )

    min_price = serializers.DecimalField(
        max_digits=10, decimal_places=2, required=False, help_text=_("Minimum monthly rent")
    )

    max_price = serializers.DecimalField(
        max_digits=10, decimal_places=2, required=False, help_text=_("Maximum monthly rent")
    )

    bedrooms = serializers.IntegerField(
        required=False, min_value=0, help_text=_("Number of bedrooms")
    )

    bathrooms = serializers.IntegerField(
        required=False, min_value=1, help_text=_("Number of bathrooms")
    )

    pets_allowed = serializers.BooleanField(
        required=False, help_text=_("Filter properties that allow pets")
    )

    parking_available = serializers.BooleanField(
        required=False, help_text=_("Filter properties with parking")
    )

    furnishing_status = serializers.ChoiceField(
        choices=Rental.FURNISHING_STATUS, required=False, help_text=_("Filter by furnishing status")
    )

    utilities_included = serializers.BooleanField(
        required=False, help_text=_("Filter properties with utilities included")
    )

    max_distance_to_campus = serializers.FloatField(
        required=False, min_value=0, help_text=_("Maximum distance to campus in miles")
    )

    shuttle_service = serializers.BooleanField(
        required=False, help_text=_("Filter properties with shuttle service")
    )

    available_from = serializers.DateField(required=False, help_text=_("Available from date"))
    ...
    radius = serializers.FloatField(
        required=False, min_value=0.1, max_value=50, help_text=_("Search radius in miles")
    )
