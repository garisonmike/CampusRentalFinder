...
class Rental(models.Model):
    ...
    title = models.CharField(
        _("title"), max_length=200, help_text=_("Attractive title for the rental property")
    )

    description = models.TextField(
        _("description"), help_text=_("Detailed description of the property")
    )

    property_type = models.CharField(
        _("property type"), max_length=20, choices=PROPERTY_TYPES, default="apartment"
    )
    ...
    address = models.CharField(
        _("address"), max_length=255, help_text=_("Street address of the property")
    )
    ...
    furnishing_status = models.CharField(
        _("furnishing status"), max_length=20, choices=FURNISHING_STATUS, default="unfurnished"
    )

    # Features and Amenities
    parking_available = models.BooleanField(_("parking available"), default=False)
    ...
    laundry_available = models.BooleanField(_("laundry available"), default=False)

    internet_included = models.BooleanField(_("internet included"), default=False)

    gym_access = models.BooleanField(_("gym access"), default=False)

    pool_access = models.BooleanField(_("pool access"), default=False)

    # Availability
    available_from = models.DateField(
        _("available from"), help_text=_("Date when the property becomes available")
    )
    ...
    status = models.CharField(
        _("rental status"), max_length=20, choices=RENTAL_STATUS, default="available"
    )
    ...
    contact_phone = models.CharField(
        _("contact phone"), max_length=17, blank=True, help_text=_("Phone number for inquiries")
    )
    ...
    shuttle_service = models.BooleanField(_("shuttle service to campus"), default=False)

    # Metadata
    views_count = models.PositiveIntegerField(
        _("views count"), default=0, help_text=_("Number of times this listing has been viewed")
    )
    ...
    def is_available(self):
        ...
        return self.status == "available" and self.available_from <= date.today()
...
class RentalImage(models.Model):
    ...
    image = models.ImageField(
        _("image"), upload_to=rental_image_upload_path, help_text=_("Property image")
    )

    caption = models.CharField(
        _("caption"), max_length=200, blank=True, help_text=_("Optional caption for the image")
    )
    ...
    order = models.PositiveIntegerField(
        _("display order"), default=0, help_text=_("Order in which images should be displayed")
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if self.is_primary:
            ...
            RentalImage.objects.filter(rental=self.rental, is_primary=True).exclude(
                pk=self.pk
...
class RentalFavorite(models.Model):
    ...
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="favorite_rentals"
    )

    rental = models.ForeignKey(Rental, on_delete=models.CASCADE, related_name="favorited_by")

    created_at = models.DateTimeField(_("favorited at"), auto_now_add=True)
...
class RentalInquiry(models.Model):
    ...
    rental = models.ForeignKey(Rental, on_delete=models.CASCADE, related_name="inquiries")
    ...
    preferred_move_date = models.DateField(_("preferred move-in date"), null=True, blank=True)

    status = models.CharField(_("status"), max_length=10, choices=INQUIRY_STATUS, default="new")

    landlord_reply = models.TextField(_("landlord reply"), blank=True)
