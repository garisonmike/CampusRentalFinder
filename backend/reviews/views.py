...
class ReviewViewSet(ModelViewSet):
    ...
    queryset = Review.objects.select_related("tenant", "rental").filter(is_approved=True)
    ...
    @extend_schema(
        ...
        parameters=[
            OpenApiParameter(
                "rental_id", OpenApiTypes.INT, description="Filter by rental property"
            ),
            ...
            OpenApiParameter(
                "verified_only", OpenApiTypes.BOOL, description="Show only verified reviews"
            ),
            OpenApiParameter(
                "recommended_only",
                OpenApiTypes.BOOL,
    ...
    @extend_schema(
        summary="Update review", description="Update review (Author only)", tags=["Reviews"]
    )
    ...
    @extend_schema(
        summary="Delete review", description="Delete review (Author only)", tags=["Reviews"]
    )
    ...
    def vote_helpfulness(self, request, pk=None):
        ...
        serializer = ReviewHelpfulnessSerializer(
            data={"review": review.id, **request.data}, context={"request": request}
        )
    ...
    @extend_schema(
        summary="Get user's reviews", description="Get current user's reviews", tags=["Reviews"]
    )
    def my_reviews(self, request):
        ...
        reviews = Review.objects.filter(tenant=request.user).select_related("rental")
...
class LandlordResponseView(APIView):
    ...
    def post(self, request, review_id):
        ...
        serializer = LandlordResponseSerializer(review, data=request.data, partial=True)
...
def rental_reviews(request, rental_id):
    ...
    reviews = (
        Review.objects.filter(rental=rental, is_approved=True)
...
def rental_review_statistics(request, rental_id):
    ...
    reviews = Review.objects.filter(rental=rental, is_approved=True)
    ...
    recommendation_percentage = (
        (recommended / total_with_recommendation * 100) if total_with_recommendation > 0 else 0
    )
...
def review_statistics(request):
    ...
    for i in range(12):
        ...
        monthly_stats.append(
            {"month": month_start.strftime("%Y-%m"), "reviews_created": monthly_count}
        )
...
def top_rated_reviews(request):
    ...
    reviews = (
        ...
        .select_related("tenant", "rental")
        .order_by("-rating", "-helpful_votes")[:10]
    )
...
class AdminReviewViewSet(ModelViewSet):
    ...
    def add_moderation_notes(self, request, pk=None):
        ...
        if not notes.strip():
            return Response(
                {"error": _("Moderation notes cannot be empty")}, status=status.HTTP_400_BAD_REQUEST
            )
...
class AdminReviewReportViewSet(ModelViewSet):
    ...
    def resolve(self, request, pk=None):
        ...
        if report.is_resolved:
            return Response(
                {"error": _("Report is already resolved")}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        return Response(
            {"message": _("Report resolved successfully"), "admin_action": report.admin_action}
        )
    ...
    def dismiss(self, request, pk=None):
        ...
        return Response({"message": _("Report dismissed successfully")})
