...
class Review(models.Model):
    ...
    comment = models.TextField(
        _("review comment"), max_length=1000, help_text=_("Detailed review comment")
    )
    ...
    is_approved = models.BooleanField(
        _("approved"), default=True, help_text=_("Whether this review is approved for display")
    )

    moderation_notes = models.TextField(
        _("moderation notes"), blank=True, help_text=_("Internal notes for moderators")
    )
    ...
    landlord_response_date = models.DateTimeField(
        _("landlord response date"), null=True, blank=True
    )

    # Helpfulness tracking
    helpful_votes = models.PositiveIntegerField(
        _("helpful votes"), default=0, help_text=_("Number of users who found this review helpful")
    )

    total_votes = models.PositiveIntegerField(
        _("total votes"), default=0, help_text=_("Total number of helpfulness votes")
    )
    ...
    def clean(self):
        ...
        if self.pk is None:  # New review
            existing_review = Review.objects.filter(rental=self.rental, tenant=self.tenant).exists()

            if existing_review:
                raise ValidationError({"tenant": _("You have already reviewed this property.")})
...
class ReviewHelpfulness(models.Model):
    ...
    is_helpful = models.BooleanField(
        _("is helpful"), help_text=_("Whether the user found the review helpful")
    )

    created_at = models.DateTimeField(_("voted at"), auto_now_add=True)
...
class ReviewReport(models.Model):
    ...
    is_resolved = models.BooleanField(
        _("resolved"), default=False, help_text=_("Whether this report has been resolved by admin")
    )

    admin_action = models.TextField(
        _("admin action"), blank=True, help_text=_("Action taken by admin to resolve this report")
    )
...
def update_review_helpfulness_on_save(sender, instance, created, **kwargs):
    ...
    if created:
        ...
        Review.objects.filter(id=review.id).update(
            helpful_votes=helpful_count, total_votes=total_count
        )
...
def update_review_helpfulness_on_delete(sender, instance, **kwargs):
    ...
    Review.objects.filter(id=review.id).update(helpful_votes=helpful_count, total_votes=total_count)
