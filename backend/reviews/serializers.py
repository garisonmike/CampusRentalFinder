...
class ReviewListSerializer(serializers.ModelSerializer):
    ...
    def get_user_found_helpful(self, obj):
        ...
        if request and request.user.is_authenticated:
            vote = ReviewHelpfulness.objects.filter(review=obj, user=request.user).first()
...
class ReviewCreateSerializer(serializers.ModelSerializer):
    ...
    def validate_move_out_date(self, value):
        ...
        if value and value > date.today():
            raise serializers.ValidationError(_("Move-out date cannot be in the future."))
        return value
...
class LandlordResponseSerializer(serializers.ModelSerializer):
    ...
    def validate_landlord_response(self, value):
        ...
        if not value.strip():
            raise serializers.ValidationError(_("Response cannot be empty."))
        if len(value.strip()) < 10:
            raise serializers.ValidationError(_("Response must be at least 10 characters long."))
        return value
...
class AdminReviewReportSerializer(serializers.ModelSerializer):
    ...
    def get_review_details(self, obj):
        ...
        return {
            ...
            "comment": obj.review.comment[:100] + "..."
            if len(obj.review.comment) > 100
            else obj.review.comment,
